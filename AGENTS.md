...
**選修課（3 堂，不在主線順序內）**：補齊上游原版有、主線刻意不教的三項能力。都需要額外門檻，收尾指回 `course-structure.json` 各自的 `returnsTo`（5-4→5-2、4-6→4-7、4-7→4-3），不是一律回 4-1；`recap` 推導「下一課」時必須跳過選修課。

| 課號 | 主題 | 前置 | 門檻 |
|------|------|------|------|
| 5-4 | 真的把圖生出來 | 5-1 | Gemini 生圖無免費額度，須開通付費 |
| 4-6 | 改壞了能回頭：存進 GitHub | 4-2 | 需 GitHub 帳號；本機須有 `git` 與 `gh` |
| 4-7 | 把小工具放上網 | 4-6（硬性） | 發布站台全世界可見且付費也無法限縮，須先去識別化 |
